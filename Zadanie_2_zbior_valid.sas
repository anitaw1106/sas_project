...
title "Udzia³ braków danych dla zmiennych, które opisuj¹ stan w danym punktcie czasu dla zbioru valid";
...
* wykres 12 ;
title "Iloœæ braków dla agr, act, ags, app, def dla zbioru valid";
...
title "Iloœæ braków oraz œrednia iloœæ braków dla zbioru valid";
...
title "Iloœæ zmiennych w zale¿noœci od udzia³u brakuj¹cych wartoœci dla zbioru valid";
axis label=none;
pattern color=orange;
proc gchart data=work.proc_miss;
hbar 'Odsetek_braków'n / type=sum
space=1 width=5 outside=freq levels=10 
range noframe;
run; quit;
/* Wykres 8*/
TITLE 'Rozk³ad procentowy zmiennych na podstawie braków danych ze zbioru valid';
proc univariate data=proc_miss NOPRINT;
histogram Odsetek_braków / midpoints = 0 to 1 by 0.1 barlabel = percent;
run;
title;
/*Wykres 10*/
TITLE 'Procentowy brak zmiennych dla poszczególnych typów zmiennych ze zbioru valid';
proc sgplot data=fir_var;
vbar var_labels / response=Odsetek_braków datalabel
fillattrs=(color=orange) categoryorder=RESPdesc;
...
quit;
/*Wykres 2*/
TITLE 'Miesiêczny szereg danych czasowych zawieraj¹cy informacje o brakach danych ze zbioru valid';
